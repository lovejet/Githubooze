...
const UserRepoList = ({ user }: { user: string | undefined }) => {
  ...
  useEffect(() => {
    const api_url = API_GITHUB_USER_REPOS.replace("%1", user || "").replace(
      "%2",
      repoList.current_page.toString()
    );

    dispatch(fetchRepos(api_url));
  }, [dispatch, user, repoList.current_page]);

  return <UserRepoListContainer>{renderChild()}</UserRepoListContainer>;
};
