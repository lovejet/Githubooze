...
const GithubUsersList = () => {
    ...
    return (
      <ListContainer>
        {userList.data.map((user) => (
          <GithubUserCard key={user.node_id} user={user} />
        ))}
      </ListContainer>
    );
  };
  ...
  return (
    <GithubUsersListContainer>
      <TotalUserCount active={userList.total !== 0}>
        {userList.total} user{userList.total >= 2 ? "s" : ""} found.
      </TotalUserCount>
      {renderChild()}
    </GithubUsersListContainer>
  );
